...
class scoped_nsprotocol {
 public:
  explicit scoped_nsprotocol(NST object = nil) : object_(object) {
      NSLog(@"chengjian_test scoped_nsprotocol create object=%@,this=%p",object,this);
  }
...
//  scoped_nsprotocol(const scoped_nsprotocol<NST>& that) : object_([that.object_ retain]) {}
  ...
  scoped_nsprotocol(const scoped_nsprotocol<NSU>& that) : object_([that.get() retain]) {
      NSLog(@"chengjian_test scoped_nsprotocol retain object=%@,this=%p",that,this);
  }

  ~scoped_nsprotocol() {
      NSLog(@"chengjian_test scoped_nsprotocol release object_=%@ this=%p",object_,this);
//      NSLog(@"chengjian_test scoped_nsprotocol release object.rc=%ld", RC(object_));
      [object_ release];
  }

  scoped_nsprotocol& operator=(const scoped_nsprotocol<NST>& that) {
    NSLog(@"chengjian_test scoped_nsprotocol operator= object_=%@,rc=%ld",that.get(),RC(that.get()));
  ...
  void reset(NST object = nil) {
    ...
    [object_ release];
    object_ = object;
    NSLog(@"chengjian_test scoped_nsprotocol reset object_=%@,rc=%ld",object,RC(object_));
  }
...
class scoped_nsobject : public scoped_nsprotocol<NST*> {
 public:
  explicit scoped_nsobject(NST* object = nil) : scoped_nsprotocol<NST*>(object) {
      NSLog(@"chengjian_test scoped_nsobject create object=%@,this=%p",object,this);
  }
